...
func Post(body string) (Task, error) {
	...
	thisItem.Name = reg.ReplaceAllString(thisItem.Name, "")
...
func GetAll() []Task {
	...
	fmt.Println("Found", numtasks)
	return tasks

}

func GetAllByName(name string) []Task {
	sess := session.Must(session.NewSession())
	svc := dynamodb.New(sess)
	...
	input := &dynamodb.QueryInput{
		...
		KeyConditionExpression: aws.String("#N = :taskname"),
